...
function ComparePointsSlope(
  ...
  if (
    ...
    areFloatsSimilar(endPoint.values, middlePoint.values, differenceThreshold)
  ) {
    return { skip: true, similar: false };
  }
...
function optimizeKeyframes(
  ...
  if (optimizeSettings.optimizeSimilarPoints.active) {
    optimizers.push((a, b, c) =>
      optimizeSimilarPoints(a, b, c, optimizeSettings.optimizeSimilarPoints)
    );
  }
    ...
    if (optimizedKeyframes.length === 2) {
      toRemove.push(
        ...optimizers.map((optimizerFn) =>
          optimizerFn(optimizedKeyframes[0], optimizedKeyframes[1], undefined)
        ),
      );
    }
