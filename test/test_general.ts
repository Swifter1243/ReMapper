...
describe('General', function () {
    ...
    it('toDegrees()', function () {
        assert.deepStrictEqual(toDegrees([0, Math.PI, 3 * Math.PI]), [
            0,
            180,
            540,
        ])
    ...
    it('toRadians()', function () {
        assert.deepStrictEqual(toRadians([0, 180, 540]), [
            0,
            Math.PI,
            3 * Math.PI,
        ])
    ...
    it('arrEqual()', function () {
        assert.strictEqual(
            arrEqual([0.1, 0.3, 0.5, 0.7], [0.6, 0.4, 1.1, 42], 0.5),
            false,
        )
        assert.strictEqual(
            arrEqual([0.1, 0.3, 0.5, 0.7], [0.6, 0.4, 0.4, 0.4], 0.5),
            true,
        )
        assert.strictEqual(arrEqual([1, 2, 3, 4], [2, 3, 4, 5], 1), true)
        assert.strictEqual(arrEqual([1, 2, 3, 4], [2, 3, 4, 2], 1), false)
        assert.strictEqual(
            arrEqual([0, 10, 100, 1000], [0, 10, 100, 1000], 0),
            true,
        )
    })
})
